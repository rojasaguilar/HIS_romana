...
export const CreateMedicPage = () => {
  ...
  const [professionalLicenceNumber, setProfessionalLicenceNumber] = useState("");
    ...
    createMedic.mutate(
      {
        ...
        languages: languages.length > 0 ? languages.split(",").map((language) => language.trim()) : [],
        specialityIds,
        medicalSchool,
        startPracticeDate,
        bio,
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl mx-auto pb-10">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <Link to="/medics" className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-indigo-600 transition">
            <MoveLeft className="w-4 h-4" />
            Volver a Médicos
          </Link>
        </div>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Registrar Nuevo Médico</h1>
          <p className="text-sm text-gray-500 mt-1">Configura el perfil, credenciales y especialidades del profesional.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input required placeholder="Ej. Carlos Ruiz" value={name} onChange={(e) => setName(e.target.value)} className={inputClassName} />
            </div>

            <div>
              <label className={labelClassName}>Correo Electrónico</label>
              <input type="email" required placeholder="carlos@clinica.com" value={email} onChange={(e) => setEmail(e.target.value)} className={inputClassName} />
            </div>
            ...
            <div>
              <label className={labelClassName}>URL Fotografía de Perfil (Opcional)</label>
              <input placeholder="https://..." value={profilePictureUrl} onChange={(e) => setProfilePictureUrl(e.target.value)} className={inputClassName} />
            </div>

            <div className="md:col-span-2">
              <label className={labelClassName}>Idiomas</label>
              <input placeholder="Ej. Español, Inglés, Francés (Separados por coma)" value={languages} onChange={(e) => setLanguages(e.target.value)} className={inputClassName} />
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
            ...
            <h2 className="text-lg font-bold text-gray-900">Credenciales Médicas</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className={labelClassName}>Licencia de Salud</label>
              <input required placeholder="Ej. SALUD-12345" value={healthLicenseNumber} onChange={(e) => setHealthLicenseNumber(e.target.value)} className={inputClassName} />
            </div>

            <div>
              <label className={labelClassName}>Cédula Profesional</label>
              <input required placeholder="Ej. MED-67890" value={professionalLicenceNumber} onChange={(e) => setProfessionalLicenceNumber(e.target.value)} className={inputClassName} />
            </div>

            <div>
              <label className={labelClassName}>Escuela de Medicina</label>
              <input required placeholder="Ej. Universidad Nacional" value={medicalSchool} onChange={(e) => setMedicalSchool(e.target.value)} className={inputClassName} />
            </div>

            <div>
              <label className={labelClassName}>Fecha de Inicio de Práctica</label>
              <input type="date" required value={startPracticeDate} onChange={(e) => setStartPracticeDate(e.target.value)} className={inputClassName} />
            </div>

            <div>
              <label className={labelClassName}>Tipo de Médico</label>
              {/* CORRECCIÓN: Opciones ajustadas a tu MedicType real */}
              <select value={type} onChange={(e) => setType(e.target.value as MedicType)} className={inputClassName}>
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-6 border-b border-gray-100 pb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-gray-900">Especialidades y Costos</h2>
            </div>
            
            {/* Honorarios en la cabecera para destacar */}
            <div className="flex items-center gap-3">
              <label className={`${labelClassName} mb-0`}>Honorario / Comisión (%)</label>
              <input 
                type="number" 
                required 
                min="0" 
          ...
          <div>
            <label className={`${labelClassName} mb-3`}>Selecciona las especialidades</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              ...
              {specialities?.length === 0 && (
                <p className="text-sm text-gray-500 col-span-full">No hay especialidades registradas.</p>
              )}
            </div>
          </div>
        </div>
