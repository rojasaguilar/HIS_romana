...
import { format, subMonths, startOfYear, eachMonthOfInterval, isSameMonth } from "date-fns";
...
export const DashboardPage = () => {
  ...
  const getMedicName = (id: string) => medics?.find(m => m.id === id)?.name ?? "Médico";
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col min-h-[350px]">
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <p className="text-xs text-gray-500">Citas por volumen de tiempo</p>
            </div>
            
            <div className="relative inline-block text-left">
              <select 
                value={activityInterval}
                onChange={(e) => setActivityInterval(e.target.value as IntervalType)}
            ...
            {activityData.data.map((item, i) => {
              ...
              return (
                <div key={i} className="relative flex flex-col items-center flex-1 group h-full justify-end">
                  ...
                  <div 
                    style={{ height: `${item.count === 0 ? '4%' : `${height}%`}` }} 
                    className={`w-full max-w-[32px] transition-all duration-700 ease-out rounded-t-md shadow-sm ${
                      item.count === 0 ? 'bg-gray-100' : 'bg-indigo-500 hover:bg-indigo-600'
                    }`}
                  />
            ...
            {specData.sorted.map((spec, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center justify-end flex-1 group h-full">
                  ...
                  <div 
                    style={{ height: `${spec.count === 0 ? '4%' : `${height}%`}` }} 
                  ...
                  />
                  <span className="mt-3 text-[9px] font-bold text-gray-400 text-center uppercase truncate w-full" title={spec.name}>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-xs text-gray-500">Procedimientos más solicitados</p>
            </div>
            ...
            {serviceData.sorted.map((srv, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center justify-end flex-1 group h-full">
                  ...
                  <div 
                    style={{ height: `${srv.count === 0 ? '4%' : `${height}%`}` }} 
                  ...
                  />
                  <span className="mt-3 text-[9px] font-bold text-gray-400 text-center uppercase truncate w-full" title={srv.name}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="flex flex-col gap-4">
            {isLoading ? (
              <p className="p-4 text-center text-sm text-gray-400 italic">Actualizando agenda...</p>
            ) : todayAppointments.map((app) => (
              <Link key={app.id} to={`/appointments/${app.id}`} className="group flex items-center justify-between p-4 bg-gray-50/50 rounded-xl border border-transparent hover:border-indigo-100 hover:bg-white transition-all">
                <div className="flex items-center gap-4">
                  <div className="flex flex-col items-center justify-center bg-white w-14 h-14 rounded-lg border border-gray-100 shadow-sm flex-shrink-0">
                    <span className="text-[9px] font-bold text-gray-400 uppercase">{format(new Date(app.startDate), "dd MMM", { locale: es })}</span>
                    <span className="text-[11px] font-black text-indigo-600">{format(new Date(app.startDate), "HH:mm")}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-gray-500">
                      <Stethoscope className="w-3 h-3" /> Dr. {getMedicName(app.medicId)}
                      <span>•</span>
                      <Activity className="w-3 h-3" /> {getServiceName(app.serviceId)}
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="flex flex-col gap-3">
            {[{ n: "Maria G.", p: "Premium", d: 5 }, { n: "Carlos R.", p: "Básico", d: 8 }].map((s, i) => (
              <div key={i} className="flex justify-between p-4 bg-orange-50/30 border border-orange-100/50 rounded-xl">
                <div className="text-sm font-bold text-gray-800">{s.n} <span className="block text-[10px] font-medium text-gray-400">{s.p}</span></div>
                <div className="text-[10px] font-bold text-orange-600 bg-white px-2 py-1 rounded border border-orange-100">Vence en {s.d} días</div>
              </div>
            ))}
          </div>
        </div>
