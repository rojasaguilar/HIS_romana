...
export const CreateSubscriptionPage = () => {
  ...
  const selectedVariant = selectedPlan?.variants.find((v) => v.id === variantId);
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto pb-10">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <Link to="/subscriptions" className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-indigo-600 transition">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Asocia un plan de salud a un paciente de la clínica.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            <div className="flex items-center gap-2 mb-6 text-indigo-600">
              ...
              <h2 className="font-bold text-gray-900 text-lg">1. Seleccionar Paciente</h2>
            </div>
            <select
              required
              value={patientId}
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            <div className="flex items-center gap-2 mb-6 text-indigo-600">
              ...
              <h2 className="font-bold text-gray-900 text-lg">2. Elegir Plan</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {plans?.map((plan) => (
                ...
                >
                  {planId === plan.id && <CheckCircle2 className="absolute top-4 right-4 w-5 h-5 text-indigo-600" />}
                  <h3 className="font-bold text-gray-900 pr-6">{plan.name}</h3>
          ...
          {selectedPlan && (
            <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
              <div className="flex items-center gap-2 mb-6 text-indigo-600">
                ...
                <h2 className="font-bold text-gray-900 text-lg">3. Duración y Costo</h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {selectedPlan.variants.map((variant) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <span className="text-sm font-bold text-gray-900">{variant.durationInMonths} meses</span>
                      <span className="text-lg font-black text-green-600">${variant.price}</span>
                    </div>
                    <div className="mt-4 pt-4 border-t border-gray-100 space-y-2">
                      {variant.monthlyVisitsIncluded.map((service, idx) => (
                        <div key={idx} className="flex items-center gap-2 text-[11px] text-gray-600">
                          ...
                          <span className="font-medium">{service.visits}x {getServiceName(service.serviceId)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="sticky top-24 bg-indigo-900 text-white rounded-3xl p-8 shadow-xl shadow-indigo-200">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <p className="text-indigo-300 text-[10px] font-bold uppercase tracking-widest">Paciente</p>
                <p className="font-medium">{patients?.find(p => p.id === patientId)?.name || "No seleccionado"}</p>
              </div>

              <div className="space-y-2">
                <p className="text-indigo-300 text-[10px] font-bold uppercase tracking-widest">Plan Seleccionado</p>
                <p className="font-medium">{selectedPlan?.name || "Pendiente"}</p>
                {selectedVariant && (
                  <p className="text-sm text-indigo-200">{selectedVariant.durationInMonths} meses de cobertura</p>
                )}
              </div>
              ...
              <div className="pt-6 border-t border-indigo-800">
                <div className="flex justify-between items-end">
                  ...
                  <p className="text-3xl font-black text-white">${selectedVariant?.price || "0"}</p>
                </div>
              </div>

              <button
                type="submit"
                disabled={!patientId || !planId || !variantId || createSubscription.isPending}
              ...
              >
                {createSubscription.isPending ? "Procesando..." : "Confirmar Suscripción"}
