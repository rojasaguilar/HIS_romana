...
export const EncounterSection = ({
    ...
    <div className="pt-6 border-t border-gray-100">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
        <div className="bg-white border border-gray-100 p-3 rounded-xl shadow-sm flex flex-col">
          <span className="text-xs text-gray-500 font-semibold mb-1">Tensión Arterial</span>
          <span className="font-bold text-gray-900 text-lg">
            {encounter.signosVitales.tensionArterial?.sistolica}/{encounter.signosVitales.tensionArterial?.diastolica} <span className="text-xs font-normal text-gray-500">mmHg</span>
          </span>
        </div>

        <div className="bg-white border border-gray-100 p-3 rounded-xl shadow-sm flex flex-col">
          <span className="text-xs text-gray-500 font-semibold mb-1">Frec. Cardíaca</span>
          <span className="font-bold text-gray-900 text-lg">
            {encounter.signosVitales.frecuenciaCardiaca?.valor} <span className="text-xs font-normal text-gray-500">lpm</span>
          </span>
        </div>

        <div className="bg-white border border-gray-100 p-3 rounded-xl shadow-sm flex flex-col">
          <span className="text-xs text-gray-500 font-semibold mb-1">Frec. Respiratoria</span>
          <span className="font-bold text-gray-900 text-lg">
            {encounter.signosVitales.frecuenciaRespiratoria?.valor} <span className="text-xs font-normal text-gray-500">rpm</span>
          </span>
        </div>

        <div className="bg-white border border-gray-100 p-3 rounded-xl shadow-sm flex flex-col">
          <span className="text-xs text-gray-500 font-semibold mb-1">Temperatura</span>
          <span className="font-bold text-gray-900 text-lg">
            {encounter.signosVitales.temperatura?.valor} <span className="text-xs font-normal text-gray-500">°C</span>
          </span>
        </div>

        <div className="bg-white border border-gray-100 p-3 rounded-xl shadow-sm flex flex-col">
          <span className="text-xs text-gray-500 font-semibold mb-1">Peso y Talla</span>
          <span className="font-bold text-gray-900 text-md">
            {encounter.signosVitales.peso?.valor} {encounter.signosVitales.peso?.unidad} / {encounter.signosVitales.talla?.valor} {encounter.signosVitales.talla?.unidad}
          </span>
        </div>

        <div className="bg-white border border-gray-100 p-3 rounded-xl shadow-sm flex flex-col md:col-span-2">
          <span className="text-xs text-gray-500 font-semibold mb-1">Índice de Masa Corporal (IMC)</span>
          <div className="flex items-center gap-2">
            <span className="font-bold text-gray-900 text-lg">{encounter.signosVitales.imc?.valor}</span>
            <span className="px-2 py-0.5 bg-blue-50 text-blue-700 text-xs font-bold rounded-md border border-blue-100">
              {encounter.signosVitales.imc?.clasificacion || "Sin clasificar"}
            </span>
          </div>
        </div>

        <div className="bg-white border border-gray-100 p-3 rounded-xl shadow-sm flex flex-col md:col-span-2 lg:col-span-3">
          <span className="text-xs text-gray-500 font-semibold mb-1">Índice Cintura-Cadera (ICC)</span>
          <div className="flex items-center gap-2">
            <span className="font-bold text-gray-900 text-lg">{encounter.signosVitales.icc?.valor}</span>
            <span className="px-2 py-0.5 bg-orange-50 text-orange-700 text-xs font-bold rounded-md border border-orange-100">
              {encounter.signosVitales.icc?.clasificacion || "Sin clasificar"}
            </span>
          </div>
        </div>
      </div>
  ...
  {/* ======================================= */}
  {encounter.interrogatorioAparatosSistemas && encounter.interrogatorioAparatosSistemas.length > 0 && (
    <div className="pt-6 border-t border-gray-100">
      <h4 className="flex items-center gap-2 text-md font-bold text-gray-800 mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {encounter.interrogatorioAparatosSistemas.map((sistema: any, idx: number) => (
          <div key={idx} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm relative overflow-hidden">
            <div className={`absolute top-0 left-0 w-1 h-full ${sistema.normal ? 'bg-green-400' : 'bg-amber-500'}`}></div>
            <div className="flex justify-between items-start mb-2">
              <span className="font-bold text-gray-900">{sistema.system}</span>
              <span className={`text-xs font-bold px-2 py-1 rounded-md border ${sistema.normal ? 'bg-green-50 text-green-700 border-green-100' : 'bg-amber-50 text-amber-700 border-amber-100'}`}>
                {sistema.normal ? 'Normal' : 'Anormal'}
              </span>
            </div>
            {!sistema.normal && (
              <p className="text-sm text-gray-700 mb-1"><span className="font-semibold text-gray-900">Síntomas:</span> {sistema.sintomas || "No descritos"}</p>
            )}
            {sistema.notas && (
              <p className="text-sm text-gray-500 mt-2 bg-gray-50 p-2 rounded-lg">{sistema.notas}</p>
            )}
          </div>
        ))}
      </div>
  ...
  {/* EXPLORACIÓN FÍSICA */}
  {/* ======================================= */}
  {encounter.exploracionFisica && encounter.exploracionFisica.length > 0 && (
    <div className="pt-6 border-t border-gray-100">
      <h4 className="flex items-center gap-2 text-md font-bold text-gray-800 mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {encounter.exploracionFisica.map((exp: any, idx: number) => (
          <div key={idx} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm relative overflow-hidden">
            <div className={`absolute top-0 left-0 w-1 h-full ${exp.normal ? 'bg-teal-400' : 'bg-rose-500'}`}></div>
            <div className="flex justify-between items-start mb-2">
              <span className="font-bold text-gray-900">{exp.sistema}</span>
              <span className={`text-xs font-bold px-2 py-1 rounded-md border ${exp.normal ? 'bg-teal-50 text-teal-700 border-teal-100' : 'bg-rose-50 text-rose-700 border-rose-100'}`}>
                {exp.normal ? 'Sin alteraciones' : 'Con hallazgos'}
              </span>
            </div>
            {!exp.normal && exp.hallazgos && (
              <p className="text-sm text-gray-700 mb-1"><span className="font-semibold text-gray-900">Hallazgos:</span> {exp.hallazgos}</p>
            )}
            {exp.descripcion && (
              <p className="text-sm text-gray-600 mb-2">{exp.descripcion}</p>
            )}
            {exp.observaciones && (
              <p className="text-xs text-gray-500 mt-2 bg-gray-50 p-2 rounded-lg italic">Obs: {exp.observaciones}</p>
            )}
          </div>
        ))}
      </div>
  ...
  {/* ======================================= */}
  {encounter.integracionDiagnostica && encounter.integracionDiagnostica.length > 0 && (
    <div className="pt-6 border-t border-gray-100">
      <h4 className="flex items-center gap-2 text-md font-bold text-gray-800 mb-4">
      ...
      <div className="flex flex-col gap-3">
        {encounter.integracionDiagnostica.map((diag: any, idx: number) => (
          <div key={idx} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="font-bold text-gray-900 text-base">{diag.diagnostico}</span>
                {diag.cie10 && (
              ...
              {diag.observaciones && <p className="text-sm text-gray-500 mt-1">{diag.observaciones}</p>}
            </div>
            <div className="flex items-center gap-2">
              <span className={`text-xs font-bold px-3 py-1 rounded-full border ${diag.tipo === 'DEFINITIVO' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
                {diag.tipo}
              </span>
          ...
          >
            ...
            <div className="flex justify-between items-start gap-2">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-gray-500 capitalize">{p.tipo} • {p.routeAdministration}</span>
              </div>
            ...
            <div className="flex items-center justify-center gap-1.5 text-xs font-medium text-gray-500 mt-2 bg-gray-50 p-1.5 rounded-md border border-gray-100">
              ...
              <span>{p.fechaInicio ? new Date(p.fechaInicio).toLocaleDateString() : '--'}</span>
              <span>hasta</span>
              <span>{p.fechaFin ? new Date(p.fechaFin).toLocaleDateString() : '--'}</span>
            </div>
          </div>
        ))}
      </div>
  ...
  {/* ======================================= */}
  {/* LAB TESTS (Mantenido según tu original) */}
  {/* ======================================= */}
  <div className="pt-6 border-t border-gray-100">
    ...
    {/* FORM LAB - Lógica de UI se mantiene intacta */}
    {isAddingLabTest && (
      <div className="mb-6 border border-purple-100 bg-purple-50 rounded-2xl p-5">
        ...
        <div className="flex justify-end gap-3 mt-5">
          ...
          >
            <PlusCircle className="w-4 h-4" />
            {createLabTestMutation.isPending ? "Guardando..." : "Guardar estudio"}
          </button>
        </div>
      </div>
    )}
