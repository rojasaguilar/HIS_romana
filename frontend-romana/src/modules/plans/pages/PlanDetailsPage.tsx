...
export const PlanDetailsPage = () => {
  ...
  const inputClass =
    "w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all";
        ...
        <div className="flex items-center justify-between">
          ...
          {!isEditing ? (
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={handleSave}
                className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-sm"
              >
          ...
          >
              ...
              <div className="flex gap-4">
                <div className="text-right">
                  ...
                  {isEditing ? (
                    <input
                      type="number"
                      className="w-16 text-right font-bold text-indigo-600 bg-transparent border-b border-indigo-200"
                ...
                <div className="text-right border-l pl-4 border-gray-200">
                  ...
                  {isEditing ? (
                    <input
                      type="number"
                      className="w-20 text-right font-bold text-green-600 bg-transparent border-b border-green-200"
                      value={variant.price}
                      onChange={(e) =>
                        handleUpdateVariant(
                          variant.id,
              ...
              <div className="flex flex-col gap-3">
                {variant.monthlyVisitsIncluded.map((visit, idx) => (
                  ...
                  >
                    ...
                    <span className="px-2.5 py-1 bg-white border border-gray-200 rounded-md text-xs font-bold text-indigo-600">
                      {visit.visits} {visit.visits === 1 ? "visita" : "visitas"}{" "}
                      / mes
                    </span>
                  </div>
                ))}
                ...
                {variant.monthlyVisitsIncluded.length === 0 && (
                  <p className="text-xs text-gray-400 italic">
                    No hay servicios específicos en esta variante.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="px-6 py-3 bg-indigo-50/30 flex items-center gap-2">
              ...
              <p className="text-[10px] text-indigo-500 font-medium">
                Los servicios se editan desde el creador de planes general.
              </p>
            </div>
          </div>
        ))}
